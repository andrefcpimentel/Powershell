function IIS_StopSiteAndAppPool{
    <#
    .SYNOPSIS
    Stops a site and an application pool based on the paramters passed

    .DESCRIPTION
    Checks first if the app pool to Stop is stopped and if it is, it Stops it
    The same with the website
    
    .PARAMETER appPoolName
    Name of the application pool to Stop
    
    .PARAMETER $siteName
    Name of the website to Stop to Stop

    .EXAMPLE 
    StopOrStopSiteAndAppPool -appPoolName "DefaultAppPool" 
    ...
    try{
        ...
        if((Get-WebSiteState $siteName).Value -eq 'Started')
        {
        ...
        elseif((Get-WebSiteState $siteName).Value -eq 'Stopped')
        {
            Write "Already stopped, no action to perform."
        }
        else
        {
   ...
   ############ APPLICATION POOL #################
    ...
    try{
        ...
        elseif((Get-WebAppPoolState $appPoolName).Value -eq 'Stopped')
        {
            Write "Already stopped, no action to perform."
        }
        else
